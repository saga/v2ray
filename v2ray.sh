...
if [[ -f /usr/local/bin/v2ray && -f /usr/local/etc/v2ray/config.json ]] && [[ -f $backup && -d /usr/local/etc/v2ray/saga/v2ray ]]; then
	...
	. /usr/local/etc/v2ray/saga/v2ray/tools/v1xx_to_v3xx.sh
...
if [[ $mark != "v3" ]]; then
	. /usr/local/etc/v2ray/saga/v2ray/tools/v3.sh
fi
...
shadowsocks_password_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默认密码: ${cyan}saga.com$none)"): " new_sspass
		[ -z "$new_sspass" ] && new_sspass="saga.com"
		case $new_sspass in
		*[/$]*)
			echo
...
path_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默认: [${cyan}saga$none]):")" new_path
		[[ -z $new_path ]] && new_path="saga"
...
blocked_hosts() {
	...
	while :; do
		...
		echo
		echo "反馈问题或请求拦截更多域名: https://github.com/saga/v2ray/issues"
		echo
	...
	while :; do
		...
		if [[ -z $is_xshell ]]; then
			...
			if [[ $is_xshell == [yY] ]]; then
				...
				echo
				# sz /usr/local/etc/v2ray/saga_v2ray.zip
				local tmpfile="/tmp/saga_v2ray_config_$RANDOM.json"
				...
				echo
				# echo -e "$yellow 解压密码 = ${cyan}saga.com$none"
				# echo
...
get_v2ray_config_info_link() {
	...
	echo
	create_v2ray_config_text >/tmp/saga_v2ray.txt
	local random=$(echo $RANDOM-$RANDOM-$RANDOM | base64 -w 0)
	local link=$(curl -s --upload-file /tmp/saga_v2ray.txt "https://transfer.sh/${random}_233v2_v2ray.txt")
	if [[ $link ]]; then
		echo
		echo "---------- V2Ray 配置信息链接-------------"
		echo
...
update_v2ray.sh() {
	...
	if [[ $latest_version == $_version ]]; then
		...
		echo
		cd /usr/local/etc/v2ray/saga/v2ray
		git pull
...
menu() {
	...
	while :; do
		...
		echo
		echo "反馈问题: https://github.com/saga/v2ray/issues"
		echo
